...
- [Gate 5: Minecraft lifecycle acceptance](docs/gates/05-minecraft-lifecycle.md)
...
agent sandboxのPodman storage lock拒否と`absent`誤分類に対して、agent 0.3.3ではstorage障害を
独立して報告できるようにしました。しかし書き込み先の個別許可はNetavarkのlockを漏らしたため、
agent 0.3.4ではfilesystem sandboxを外し、Podmanの動的なruntime領域を制限しないservice設定へ
修正しています。
0.3.4での実account live acceptanceは未実施です。
