...
2. `itzg/minecraft-server` Quadletのstart、readiness、graceful stopを接続する。（自動test完了）
3. snapshot commit後だけLinodeを削除するworkflowを完成させる。（Gate 4 fixtureで実装）
4. 手動snapshotのquiesce/resumeと中断復旧を追加する。（自動test完了）
5. fresh LinodeへのMinecraft再restore/restartを確認する。（Gate 5 harness実装、live acceptance待ち）
...
実accountのGate 3 acceptanceも完了し、Gate 4のdata lifecycleも3台のfresh Linodeによるlive
acceptanceまで完了しています。Gate 5のMinecraft lifecycleは実装と自動testが完了し、2台のfresh
Linodeを使うlive acceptance待ちです。中期目標全体の
