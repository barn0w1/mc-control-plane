...
選択したprovider plan IDと、そのplanから計算したallocatable capacityは、external createを呼ぶ前にControl Plane内部のHost recordへ永続化します。
再起動やcreate retryでcatalogを再選択せず、同じHost IDには同じ永続化済みplanを使用します。provider固有plan IDはHostClaim specやpublic Host resourceへ公開せず、provider adapterへ渡す内部stateとして扱います。
...
Host internal state
  provider_plan_id
  retry attempt and next reconcile time
  classified last error
```

- Host IDとselected provider planはprovider create前に同じtransactionで保存する
