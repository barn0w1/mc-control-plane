...
- Host agentのfilesystem sandboxはControl Plane dataに加え、rootful Podmanのgraphroot、runroot、
  Podman/crun runtime領域だけを書き込み可能にする。揮発directoryはbootごとに作成し、Podman local
  storage障害とcontainer不存在を別状態として扱う。
