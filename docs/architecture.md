...
手動作成済みFirewallの参照など、Control Planeが必要とするHost実行条件を表します。

### MinecraftSpec

Control Planeがworkloadを再現するために必要な、digest固定container image、exact Minecraft version、
Paper build、memory、EULA acceptanceを持つvalue objectです。Paper設定、plugin、world内容は含めません。
`RuntimeSpec`と`MinecraftSpec`はRun開始時にeffective specとしてコピーし、実行途中の設定変更が
既存Runへ暗黙に影響しないようにします。

### Run

一つのServer Unitを一つのLinodeで実行する試行です。固有の`run_id`、開始時の
`RuntimeSpec`、`MinecraftSpec`、復元元snapshot ID、開始・終了時刻を持ちます。
...
- 未完了Operation中の別要求は中断・予約せず、現在のOperation情報を返して拒否する。
...
- Host agentはrootful Podmanの管理processなので、Podmanの動的なgraphroot、runroot、network lock、
  OCI runtime領域をsystemd filesystem namespaceで制限しない。command schema、認証、Run/所有権検証を
  管理面の境界とし、Podman local storage障害とcontainer不存在を別状態として扱う。
