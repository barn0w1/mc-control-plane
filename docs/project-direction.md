...
Akamai Cloud上のGNU/Linux実行環境について、HostClaimから確保、認証、観測、解放、再利用、通常削除までを所有します。
...
process再起動や同じreconciliationの再実行を通常動作として扱います。
一方、Akamai API failure、外部mutationの結果不明、ownership不一致など、正常lifecycleで解決できないfailureは共通の`FatalIncident`として永続化し、affected scopeへの自動mutationを停止します。
個別Controllerが独自の無期限retry、replacement、forced cleanupを実装しません。

共通failure modelは[Failure model](failure-model.md)、Host管理の完成形は[Host Control System v1 architecture](checkpoints/host-control-v1/architecture.md)を参照してください。
...
- Incident acknowledgementとresolution RPC
- Host layerより後のdatabase schema
- 将来のData、Workload、Server layerのresource model

## Explicitly outside the system

課金resourceの外部監視、Fatal Incident後のforced deletion、account-wideなcost enforcementはControl Planeへ組み込みません。
必要なら独立したprogramとして別に構築します。
