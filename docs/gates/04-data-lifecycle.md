...
secret fileはControl Plane Hostだけに置き、group/other permissionを付けない。restic repositoryは
`--insecure-no-password`で作成し、repository passwordやdata root keyを使用しない。restic format内部の
暗号化・認証は残るが、復元に必要なsecretはR2 access credentialだけである。
