...
- 以前はsnapshot作成時刻を同時に`verified_at`へ保存していた。snapshot作成成功と独立restore成功を
  区別し、fresh Hostでdigest一致を確認した時点だけを`verified_at`として保存する。
...
Gate 4はCompleteである。ただし、今回の修正後すぐにGate 5へは進めない。Infra、Host、Data基盤の
運用形を整理し、修正後の自動testとbuildを維持した状態で一度checkpointとする。
