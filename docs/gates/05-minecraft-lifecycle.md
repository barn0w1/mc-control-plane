...
Caddyが公開するagent artifactを0.3.3へ更新する。
...
今回の0.3.2 RunはMinecraft起動とhealth確認には成功したがsnapshot前であり、Gate 5の復旧点としては
使用しない。cleanupでactive Run/Operationを閉じてからDBを初期化するのが最も単純な再検証手順である。

`start_minecraft`はPaperのdownloadとhealth到達まで数分かかり得る。command stateが変わらない間も
60秒ごとに`elapsed=...s`を表示する。agent 0.3.3ではstart失敗時のsystemd/Podman状態、configured
...
`1000:1000`になり、container設定も`1000:1000`になる。agent 0.3.3ではlogに`Changing uid`、
