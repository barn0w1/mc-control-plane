...
三回目のlive試行ではこの直接non-root起動に成功し、Paper 26.2が継続してhealth `healthy`になった。
一方、Host agentの`ProtectSystem=strict` sandbox内ではPodmanが
`/var/lib/containers/storage/storage.lock`を開けず、exit 125を返した。観測処理がすべての非zero終了を
container `absent`と誤分類したため、Gateは正常なMinecraftを`starting`として中断した。

agent 0.3.3ではsystemd sandboxを解除せず、rootful Podmanが必要とする`/var/lib/containers`、
`/run/containers`、`/run/libpod`とDebian 13のOCI runtimeが使う`/run/crun`だけを書き込み可能にする。
揮発する`/run`配下はtmpfilesでbootごとに再作成する。観測前に`podman container exists`を使い、
公式の終了契約どおり0を存在、1を不存在、125をstorage障害として区別する。観測や追加診断が失敗しても
元のHost command resultをjournalへ確定し、再配送loopにしない。

四回目のlive試行では0.3.3の診断が意図どおり働き、`podman container exists`がNetavarkの
`/run/lock/netavark.lock`を開けずexit 125になったことを即座に報告した。一方、この結果からPodmanの
内部書き込み先を個別列挙するsandbox方針自体が不完全だと判断した。agent 0.3.4では
`ProtectSystem=strict`、Podman用`ReadWritePaths`、Podman用tmpfilesを削除し、filesystem namespace
自体を使用しない。rootful Podmanが使うHost filesystemは通常どおり書き込み可能で、
その内部directoryとlockはPodman自身に管理させる。`NoNewPrivileges`と厳しいumaskは維持する。
Gate 5は各fresh Hostの接続直後に`inspect_host`を実行し、Podman capabilityとMinecraft状態を
観測できることを確認してからrepository初期化へ進む。
...
Caddyが公開するagent artifactを0.3.4へ更新する。
...
mc-control-plane.hss-science.org {
	@allowed <<CEL
        ...
        ||
        (method('GET') && path('/artifacts/mccp-host-agent-0.3.4.whl'))
    CEL
...
失敗したRunのLinodeが残っている場合は、DBを削除する前に後述の`linode-gate3-cleanup`を実行する。
0.3.2 RunはMinecraft起動とhealth確認には成功したがsnapshot前であり、0.3.3 RunはMinecraft適用前に
停止した。どちらもGate 5の復旧点として使用しない。cleanupでactive Run/Operationを閉じてからDBを
初期化するのが最も単純な再検証手順である。

`start_minecraft`はPaperのdownloadとhealth到達まで数分かかり得る。command stateが変わらない間も
60秒ごとに`elapsed=...s`を表示する。agent 0.3.4ではstart失敗時のsystemd/Podman状態、configured
...
`1000:1000`になり、container設定も`1000:1000`になる。agent 0.3.4ではlogに`Changing uid`、
...
- [Podman container exists exit contract](https://docs.podman.io/en/stable/markdown/podman-container-exists.1.html)
