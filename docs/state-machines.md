...
Host stateはLinodeのstatusとは独立しています。Linodeが`running`でもcloud-initやPodmanの準備が完了していない場合があります。

```mermaid
stateDiagram-v2
    state "到達不能" as Unreachable
    state "bootstrap中" as Bootstrapping
    state "準備完了" as Ready
    state "失敗" as Failed

    [*] --> Unreachable
    Unreachable --> Bootstrapping: Hostから応答
    Bootstrapping --> Ready: 必要条件を確認
    Bootstrapping --> Failed: cloud-init失敗
    Ready --> Unreachable: VM停止・通信断
    Failed --> Bootstrapping: 明示的retry
```

`ready`の判定条件には、少なくともdata directory、Podman、Minecraft image、
restic、Control Planeとの通信手段が使用可能であることを含めます。
具体的なhost protocolは実装段階で決めます。
