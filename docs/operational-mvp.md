...
- Gate 2 harnessの実accountによるlive acceptance
- start workflowの`WAIT_HOST`とHost observationの接続
...
1. Project ownerがGate 2 live acceptanceを実行し、結果を記録する。
2. Host observationをstart workflowの`WAIT_HOST`へ接続する。
3. Gate 3の単一reconcilerと操作・状態表示CLIを実装する。
4. その後にrestic、Minecraftの順で進める。
