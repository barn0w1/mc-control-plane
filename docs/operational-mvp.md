...
1. Gate 4 live acceptanceで発見したR2権限、command delivery、SQLite競合、snapshot検証時刻を
   regression testと文書へ固定する。（完了）
2. Gate 1から4までをInfra/Host/Data基盤のcheckpointとしてbuild可能な状態に保つ。
3. Control PlaneとHost APIの常駐方法、configuration/secret fileの配置を次の短期計画で決める。
4. そのcheckpointを確認するまでGate 5のMinecraft lifecycleへ進まない。
