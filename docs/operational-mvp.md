...
Status: **Foundation acceptance complete / product workflow pending**（2026-07-23）。Gate 1から5で
技術的なend-to-end lifecycleは実account検証済みである。通常運用向けの個別Start、Stop、Snapshot
OperationとCLIへ同じprimitiveを接続した時点でOperational MVP全体をCompleteとする。
...
- 通常運用の永続Stop Operationと`server-unit-stop`
- 通常運用の手動snapshot requestとsnapshot一覧
- start workflow内のrestore、Minecraft apply/start、readyへの接続
...
Gate 5 harnessは1、2、3、5、6、7に相当するprimitiveと順序を実機で証明した。ただし、これらを
一つのacceptance commandではなく通常の個別CLIと永続Operationから実行できることが、この節の残る
完成条件である。Control Plane再起動はGate 3、Host/VM再接続はGate 2で独立して検証済みである。
...
4. 検証済みprimitiveを通常のStart、Stop、Snapshot OperationとCLIへ接続する。
5. 外部playability、VM reboot recovery、定期snapshotの優先順位を改めて決める。
