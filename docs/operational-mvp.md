...
- 実accountを使うopt-in Linode lifecycle test
- cloud-init metadataの生成とLinode createへの接続
...
1. Gate 1のopt-in Linode integration harnessとcloud-init metadata入力を実装する。
2. Host agentのprotocol contract、enrollment model、Python 3.13 package骨格を実装する。
3. Debian 13 cloud-initとfixture Quadletをlocal VMまたはtest Linodeで検証する。
4. Host observationを永続化し、start workflowの`WAIT_HOST`を接続する。
5. その後にreconciler常駐process、restic、Minecraftの順で進める。
