...
Status: **Product workflow implemented / normal-operation live acceptance pending**（2026-07-23）。
Gate 1から5で技術的なend-to-end lifecycleは実account検証済みであり、通常運用向けの個別Start、
Stop、Snapshot OperationとCLIへの接続、自動scenario testも完了した。通常CLIによる同じ一周を
実accountで確認した時点でOperational MVP全体をCompleteとする。
...
- Server Unit専用のpasswordless restic/R2 repository、temporary credential data lease
- snapshot commit後のLinode削除と3台のfresh Hostを使うrestore/modify/restore lifecycle
- R2 credential発行preflight、data lease成功後のcommand delivery、SQLite busy bounded retry
- 固定Paper Quadlet、health-based readiness、graceful stop、安全な手動snapshot command
- 2台のfresh HostでMinecraftのstop snapshotをrestore/restartするGate 5 harness
- Minecraftを固定non-login UID/GIDで直接起動し、entrypointのUID変更、`gosu`、暗黙chownを回避する境界
- rootful Podman管理agentに不要なfilesystem sandboxを設けず、Host接続直後にPodmanをpreflightする構成
- `MinecraftSpec`をServer UnitとRunへ保存し、最新snapshot選択を含む永続Start workflow
- quiesced manual snapshotをcommitする永続Snapshot workflowとsnapshot一覧query
- graceful stop、停止snapshot commit、所有Linode削除確認、Run終了を行う永続Stop workflow
- CLIと将来のDiscord Botが共有するapplication command/query、競合Operationの構造化error
- blocked Operationの明示retryとattempt単位の決定論的Host command ID
- strict TOML node config、local preflight、stable agent artifact endpoint
- systemd管理の独立Host API/reconciler serviceと短縮日常CLI

start workflowはLinodeを一度だけ作成し、Host ready、restoreまたはrepository初期化、Quadlet適用、
Paper readinessまで永続stepで進む。Stop/Snapshotを含む通常の一周をfake adapterで自動testしている。

### 次段へ進む前に維持する境界

- Gate 1から5の自動test、build、live acceptance手順
- Control Plane process配置とSQLite fileのlocal filesystem運用
- R2/Cloudflare/Akamai credentialの非secret診断と課金前preflight

### 未実装・未検証

- 通常CLIによるStart/Snapshot/Stop一周の実account acceptance
- Discord Bot adapter
...
Status: **Complete**（2026-07-23）。3台のfresh Linodeによるsnapshot、明示restore、fixture変更、
再snapshot、独立restore、全resource削除を実accountで確認した。実行手順、実測値、発見した問題と
修正は[Gate 4 acceptance](gates/04-data-lifecycle.md)に固定する。
...
Status: **Complete**（2026-07-23）。専用の非login identityによる直接non-root起動、Podman
preflight、2台のfresh LinodeでのPaper ready、quiesced manual snapshot、graceful stop、停止後
snapshot、内容一致restore、再起動、全resource削除を実accountで確認した。失敗した複数の試行と
最終実測値は[Gate 5 acceptance](gates/05-minecraft-lifecycle.md)に固定する。
...
- 手動snapshotではMinecraftをquiesceし、成功・失敗・agent中断のいずれでもresume可能にする。
- 新しいLinodeで最後にcommitしたsnapshotをrestoreし、同じServer Unitを再起動できる。
- 一つのServer Unitにactive Runが最大一つであることを実環境を含むscenarioで確認する。

定期snapshotは実行間隔、失敗表示、retentionとの関係が未確定なためGate 5の完成条件に含めない。
手動snapshot primitiveのlive acceptance後に独立して設計する。
...
5. 実行中に手動snapshotを作成し、snapshot IDを記録する。
...
Gate 5 harnessは1、2、3、5、6、7に相当するprimitiveと順序を実機で証明した。通常の個別CLIと
永続Operationへの接続は自動scenario testまで完了し、この節の残る完成条件は同経路の実account
acceptanceである。Control Plane再起動はGate 3、Host/VM再接続はGate 2で独立して検証済みである。
...
1. Gate 1から4までをInfra/Host/Data基盤のcheckpointとしてbuild可能な状態に保つ。（完了）
2. Gate 5のPaper lifecycle、手動snapshot、fresh Host restartを実装し自動testする。（完了）
3. project ownerがGate 5 live acceptanceを実行し、結果を文書へ固定する。（完了）
4. 検証済みprimitiveを通常のStart、Stop、Snapshot OperationとCLIへ接続する。（自動test完了）
5. Control Plane processをsystemd service化し、TOML configと短縮CLIへ接続する。（自動test完了）
6. 通常CLIで実account lifecycleを一周し、Operational MVPを判定する。
7. 外部playability、VM reboot recovery、Discord adapter、定期snapshotの優先順位を改めて決める。
