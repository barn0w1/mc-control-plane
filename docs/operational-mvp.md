...
- Gate 1から4の自動test、build、live acceptance手順
...
- 定期snapshotとschedule
- Gate 5の実account live acceptance
...
Status: **Implementation revised / live acceptance pending**（2026-07-23）。最初のlive試行で
見つかったownership停止点を専用の非login identityと起動前検証で修正した。自動testと実行手順は
[Gate 5 acceptance](gates/05-minecraft-lifecycle.md)に固定する。
...
3. project ownerがGate 5 live acceptanceを実行し、結果を文書へ固定する。
4. Gate 5完了後に、常駐process運用、外部playability、VM reboot recovery、定期snapshotの順序を
   再評価する。
