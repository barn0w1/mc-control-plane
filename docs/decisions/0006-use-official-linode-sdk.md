...
- 作成時には手動のbreak-glass調査用としてControl Plane設定から渡されたSSH公開鍵を使い、
  root passwordを生成・保存しない。通常のHost workflowはSSHを使用しない。
- 手動作成済みFirewall IDだけを参照し、VolumeやCloud Firewall自体は作成しない。
- account既定のinterface方式へ依存せず、public Linode interfaceを明示し、そのinterfaceへ
  手動作成済みFirewall IDを関連付ける。Gate 1ではLinode直下のlegacy endpointではなく、
  各Linode InterfaceのFirewall endpointから実際の関連付けを再観測する。
- cloud-init user dataはMetadata objectとしてbase64 encodingしてcreateへ渡し、作成後に
  `has_user_data`を観測する。
- test resourceには完全なownership tagに加えてUTCの期限tagを付ける。期限は回収の手掛かり
  だけに使い、削除許可には使わない。
...
- delete adapter自身が直前に完全なownership tagを確認する。上位workflowやCLIの検査だけに
  依存しない。
- local disk encryptionは[ADR-0009](0009-disable-local-disk-encryption.md)に従って明示的に
  無効化し、作成後にもprovider値を確認する。
...
- Metadataの存在は確認できるが、cloud-init処理内容の完了はHost foundationで別途観測する。
  通常のHost制御方式は[ADR-0008](0008-use-outbound-host-agent.md)で決定した。
- 実accountに対するcreate/delete acceptanceはcredentialと課金を伴うため通常testには含めず、
  明示flagを要求する専用CLIとして実行する。
