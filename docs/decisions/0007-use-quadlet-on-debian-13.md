...
- Minecraft Quadletをboot時に無条件で開始しない。Host agentがdata stateを確認した後で
  ...
  再確認してから再開する。
- Host agentの`ProtectSystem=strict`は維持する。rootful Podman CLIはread-onlyな`inspect`でもlocal
  storage lockを使用するため、既定graphroot `/var/lib/containers`、runroot `/run/containers`、
  runtime一時領域`/run/libpod`、crun state `/run/crun`だけを`ReadWritePaths`へ追加する。
  tmpfs上の`/run` directoryはtmpfilesでbootごとに再作成する。
