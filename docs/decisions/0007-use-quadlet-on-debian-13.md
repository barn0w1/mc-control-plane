...
Gate 5のlive acceptanceでownership変換が起動停止点になったため、Minecraftには固定された専用
identityを用意し、itzgによる起動時chownを行わない。rootful Podmanを維持しつつJava processを
非rootにする境界は[ADR-0015](0015-use-dedicated-minecraft-identity.md)で固定する。
