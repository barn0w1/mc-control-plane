...
- repositoryは`--insecure-no-password`で初期化し、すべてのrepository commandで同じflagを指定する。
- restic repository format固有の暗号化は無効化できないため残るが、復元に別管理のpasswordを要求しない。
...
- 重複排除、圧縮、snapshot、検証を自作しなくてよい。
- R2 credential以外の復元secretを持たず、password紛失による復元不能を避けられる。
- R2への転送量を抑えられる。
...
- 空passwordのため、R2 objectを読める主体に対するresticの機密性は期待しない。
- restic repository formatの暗号化処理自体は無効化できない。
