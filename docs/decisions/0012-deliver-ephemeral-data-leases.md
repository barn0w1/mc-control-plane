...
- HostCommandの`delivered`遷移と`delivery_count`更新はcredential発行成功後に行う。発行失敗中は
  commandを`pending`に保ち、HTTP 503でagentへpoll再試行を求める。
...
- `host-api-serve`はR2設定時にtemporary credentialを一つ発行するpreflightをlisten開始前に行う。
- lease TTLはcommand deadlineより最低60秒長くする。defaultはcommand 1800秒、lease 3600秒とする。
...
- Cloudflare拒否時はHTTP statusとAPI error code/messageだけをControl Plane logへ出し、token、親key、
  発行されたcredentialは出さない。
